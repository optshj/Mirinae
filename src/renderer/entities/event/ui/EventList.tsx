...
export function EventList({ seg, weekStart, onDoubleClick, onPointerDown, dimmed = false, interactive = true }: EventListProps) {
  ...
  return (
    <div
      className={`${interactive ? 'pointer-events-auto' : 'pointer-events-none'} flex max-h-5 items-center overflow-hidden text-sm md:bg-(--event-color)/20 dark:saturate-70 event-color-${event.colorId} ${seg.isStart && 'rounded-l-sm'} ${seg.isEnd && 'rounded-r-sm'} ${dimmed ? 'opacity-30' : 'opacity-100'} ${isDraggable && 'cursor-grab'}`}
      style={{
