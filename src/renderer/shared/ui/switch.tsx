export function Switch({ onClick, isOn, id }: { onClick: () => void; isOn: boolean; id?: string }) {
  return (
    ...
    >
      <div className={`absolute h-5 w-5 transform rounded-full bg-white p-1 shadow transition-all duration-300 group-active:scale-80 ${isOn ? 'translate-x-3' : '-translate-x-3'} `} />
    </button>
  );
}
