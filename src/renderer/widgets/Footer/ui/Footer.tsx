...
export function Footer() {
  ...
  return (
    <footer className="mt-2 grid h-48 shrink-0 grid-cols-3 gap-2 transition-all duration-300 ease-in-out [html.flip-footer_&]:pointer-events-none [html.flip-footer_&]:invisible [html.flip-footer_&]:mt-0 [html.flip-footer_&]:h-0 [html.flip-footer_&]:opacity-0">
      <FooterEvent items={todayEvent} title="오늘 일정" description="오늘의 일정이 없습니다" />
      <FooterEvent items={upcomingEvent} title="다가오는 일정" description="다가오는 일정이 없습니다" />
      <FooterEvent items={importantEvent} title="색상별 일정" description="선택한 색상의 일정만 표시됩니다" headerButton={<PalletteDropdown colorId={colorId} setColorId={setColorId} />} />
    </footer>
  );
}
